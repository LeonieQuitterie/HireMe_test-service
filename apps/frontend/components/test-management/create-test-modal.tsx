...
import { CreateTestData, Question, Test } from "@/app/types/test";

// Type riêng cho question trong form (có id optional)
type EditableQuestion = {
    id?: string;                    // có khi edit, không có khi tạo mới
...
export function CreateTestModal({
    ...
    const fetchTestDetails = async (testId: string) => {
        ...
        try {
            ...
            if (result.success && result.data.test) {
                ...
                // GIỮ LẠI ID ĐỂ DÙNG KEY ỔN ĐỊNH
                const loadedQuestions: EditableQuestion[] = (test.questions || []).map((q: any) => ({
                    id: q.id,
    ...
    const removeQuestion = (index: number) => {
        setQuestions((prev) =>
            prev
                .filter((_, i) => i !== index)
                .map((q, i) => ({ ...q, order: i })) // cập nhật lại order
        );
    };
    ...
    const handleSubmit = async () => {
        ...
        if (passingScore < 0 || passingScore > 100)
            return alert("Passing score must be between 0 and 100");
        if (questions.length === 0) return alert("Please add at least one question");
        if (questions.some((q) => !q.question_text.trim()))
            return alert("Please fill in all question texts");
        ...
        try {
            ...
            const testData: CreateTestData = {
                ...
                questions: questions.map((q, index) => ({
                    ...
                    order: index,
                    ...(q.id && { id: q.id }), // gửi id lên để backend update câu hỏi cũ
                })),
            };

            const url = editingTest
                ? `${process.env.NEXT_PUBLIC_API_URL}/api/tests/${editingTest.id}`
                                    ...
                                    {questions.length === 0 ? (
                                        ...
                                        <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                                            {questions.map((question, idx) => {
                                                const questionKey = question.id ?? `new-${idx}`;

                                                return (
                                                    <div
                                                        key={questionKey} // KEY ỔN ĐỊNH – FIX HOÀN TOÀN LỖI MẤT NỘI DUNG
